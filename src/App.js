      ...
      <div className="md:w-1/3 max-w-sm">
        ...
        <div className="flex justify-between mt-4">
          {[1, 2, 3, 4, 5, 6].map(index => (
            <input
              key={index}
              className="otp-box text-sm w-12 h-12 px-2 py-2 border border-solid border-gray-300 rounded text-center mb-4 md:mb-0 md:mr-2"
              type="text"
              maxLength={1}
              autoFocus={index === 1 ? true : false} // Autofocus on the first box
              placeholder="0"
            />
          ))}
        </div>
